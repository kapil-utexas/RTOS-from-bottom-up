...
extern uint32_t eventLogger[100][2];
	...
	while(1){
		...
		switch(commandChosen)
		{
			...
			case '9':
				for(int i = 0; i<100; i++)
				{
					UART_OutString("ID and timestamp: ");
					UART_OutUDec(eventLogger[i][0]);  //store id of thread that will start
					UART_OutChar(' ');
					UART_OutUDec(eventLogger[i][1]);
					OutCRLF();
				}
				break;
			default:
