...
uint32_t eventLogger [100][2];
uint32_t eventLoggerElement = 0;
...
void removeAndAddToSingleList(struct PeriodicThread ** from, unsigned long  period, void(*task)(void), unsigned long priority, uint32_t id)
{
...
int OS_AddPeriodicThread(void(*task)(void), unsigned long period, unsigned long priority){
	...
	removeAndAddToSingleList(&DeadPeriodicPt, period, task, priority, uniqueId);
	uniqueId++;
	EndCritical(status );
	...
	if(PeriodPt != '\0') //at least one periodic thread has been added
	{
		while(tempTraversal != '\0')
		{
			...
			if((*tempTraversal).timeLeft == 0) //need to run!
			{
				...
				if(eventLoggerElement < 100)
				{
					eventLogger[eventLoggerElement][0] = tempTraversal->id; //store id of thread that will start
					eventLogger[eventLoggerElement][1] = timerCounter; //currentTime
					eventLoggerElement++;
				}
...
// output: none
void OS_Suspend(){
		if(eventLoggerElement < 100 && RunPt != '\0')
		{
			eventLogger[eventLoggerElement][0] = RunPt->nextTCB->id; //store id of thread that will start
			eventLogger[eventLoggerElement][1] = timerCounter; //currentTime
			eventLoggerElement++;
		}
